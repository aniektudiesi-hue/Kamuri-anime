...
export function staticSitemapRoutes(now = new Date()): MetadataRoute.Sitemap {
  return [
    {
      url: absoluteUrl("/"),
      lastModified: now,
...
export async function animeSitemapRoutes(now = new Date()): Promise<MetadataRoute.Sitemap> {
...
export async function watchSitemapRoutes(now = new Date()): Promise<MetadataRoute.Sitemap> {
  const catalog = await getHomeAnimeCatalog();
  return (
    await Promise.all(
