  ...
  return (
    <>
      ...
      <main className="h-[calc(100dvh-72px)] overflow-hidden bg-[#03040a] px-2 py-2 text-white sm:px-4 sm:py-4">
        <section className="mx-auto grid h-full max-w-screen-2xl overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#070913] shadow-[0_30px_110px_rgba(0,0,0,0.6)] lg:grid-cols-[360px_1fr]">
          <aside className="flex min-h-0 flex-col border-b border-white/[0.08] lg:border-b-0 lg:border-r">
            ...
            <div className="no-scrollbar flex shrink-0 gap-2 overflow-x-auto border-b border-white/[0.08] p-3 lg:block lg:max-h-[260px] lg:space-y-2 lg:overflow-y-auto">
              {roomItems.map((item) => {
                const name = text(item, "room", "global");
                const active = room === name;
                return (
                  <button key={name} type="button" onClick={() => setRoom(name)} className={cn("flex min-w-[210px] items-center gap-3 rounded-2xl p-3 text-left transition lg:w-full lg:min-w-0", active ? "bg-[#e11d48] text-white" : "bg-white/[0.045] text-white/74 hover:bg-white/[0.075]")}>
                    <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-black/20"><Hash size={16} /></span>
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-black capitalize">{label(name)}</span>
                      <span className="mt-0.5 block text-[11px] font-semibold opacity-65">{num(item, "messages")} saved messages</span>
                    </span>
                  </button>
                );
              })}
            ...
            <div className="flex shrink-0 gap-2 border-b border-white/[0.08] p-3">
          ...
          <section className="flex min-h-0 flex-col">
            <header className="flex shrink-0 items-center justify-between border-b border-white/[0.08] bg-[#090b14]/95 px-4 py-3">
              <div className="flex min-w-0 items-center gap-3">
                <span className="relative grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[linear-gradient(135deg,#e11d48,#7f1d1d)] text-sm font-black">
                  {avatar(label(room))}
                  <span className="absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-[#090b14] bg-emerald-400" />
                </span>
                <div className="min-w-0">
                  <h2 className="truncate text-lg font-black capitalize">{label(room)}</h2>
            ...
            <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto bg-[radial-gradient(circle_at_20%_0%,rgba(225,29,72,0.08),transparent_30%)] px-3 py-4 sm:px-5">
                ...
                {messages.map((item, index) => {
                  ...
                  return (
                    <div key={`${item.id ?? index}-${item.created_at}-${item.message}`} className={cn("flex gap-2", own ? "justify-end" : "justify-start")}>
                      ...
                      <div className={cn("max-w-[82%] sm:max-w-[66%]", own ? "text-right" : "text-left")}>
                        {!own ? <p className="mb-1 px-1 text-[11px] font-black text-white/42">@{item.username || "user"}</p> : null}
                        <div className={cn("inline-block rounded-[1.35rem] px-4 py-2.5 text-left text-sm font-semibold leading-5 shadow-lg", own ? "rounded-br-md bg-[#e11d48] text-white" : "rounded-bl-md border border-white/[0.07] bg-white/[0.075] text-white/92")}>
            ...
            <form onSubmit={submit} className="shrink-0 border-t border-white/[0.08] bg-[#090b14] p-3">
              <div className="flex items-end gap-2 rounded-[1.35rem] border border-white/[0.08] bg-white/[0.055] p-2">
              ...
              <p className="mt-2 text-center text-[11px] font-semibold text-white/30">Press Enter to send, Shift+Enter for a new line.</p>
            </form>
          </section>
        </section>
      </main>
