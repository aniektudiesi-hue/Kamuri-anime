...
export async function GET() {
  return keywordSitemapResponse(1);
}

function keywordSitemapResponse(part: number) {
  const now = new Date().toISOString();
  const urls = episodeKeywordRoutesForSitemapPart(part).map((route) => absoluteUrl(route.path));
