import { allEpisodeKeywordRoutes } from "@/lib/seo-keywords";
...
export async function GET() {
  ...
  const routes = allEpisodeKeywordRoutes();

  return new Response(toXml(routes.map((route) => absoluteUrl(route.path)), now), {
    headers: {
